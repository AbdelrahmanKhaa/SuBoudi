...
"""
   ...
   console.print(Panel("[bold magenta]POWERED BY ABDELRAHMAN KHALED | LINKEDIN:https://www.linkedin.com/in/abdelrahmankhaledahmed | GITHUB: -----[/bold magenta]", border_style="magenta"), "\n")
...
def main():
    ...
    if mode == "manual":
        file_name = Prompt.ask("[bold cyan] PLEASE ENTER THE NAME OF THE FOLDER[/bold cyan]").strip()
        path = os.path.join(os.path.expanduser("~/BugBounty"), file_name)
        ...
        path = os.path.join(os.path.expanduser("~/BugBounty"), website_name)
        os.makedirs(path, exist_ok=True)
